...
const default_aftee_gateway = {
    mode : "test",
    merchant_code: "mcc00029",
    user_shop_code: "owner0001",
    aftee_pub_key: 'syZA0BY4VrvTxRnmPknq2w',
    aftee_secret_key: '0d6Ftbq9LRYHrx7Kh-1GMg'
};
...
router.post('/payment-create', function (req, res, next) {
    ...
    var connect_page_id = body.cpid,
        ...
        dest_zipcode = body.dest_zipcode || '',
    ...
    getConnectPageInfo(connect_page_id, user_id, function (check, connect_page) {
        if(check){
            ...
            params.email = email;
            params.tel_num = tel_num ? tel_num.replace(/-/g, '') : '';
            params.zipcode = zipcode;

            // by dest_user
            params.dest_name = dest_name;
            params.dest_zipcode = dest_zipcode;
            params.dest_address = dest_address;
            params.dest_tel_num = dest_tel_num ? dest_tel_num.replace(/-/g, '') : '';
            ...
            getPaymentGatewayCODInfo(params, function (error, gateway) {
                ...
                params.aftee_pub_key = gateway.aftee_pub_key;
                params.aftee_secret_key = gateway.aftee_secret_key;
                params.mode = gateway.mode;
                getVariablesOrderCOD(params, function (price) {
                    execTranAftee(params, price, function (result) {
                        if(result && result.success){
                            response = {
                                aftee_np_data: result['np_data'],
                                aftee_pub_key: result['aftee_pub_key'],
        ...
        if(res){
            PaymentGateway.findOne({user_id: res.user_id, default_flg: 1}, function (e, data_gateway) {
                if(data_gateway){
                    ...
                    return callback(false, default_aftee_gateway);
                }
            });
        }else{
            return callback(true);
...
function execTranAftee(params, price, callback) {
    var options = {};
    var result = {};
    ...
    if (params.aftee_pub_key != void 0) {
        options['amount'] = 0; // 課⾦額
        options['sales_settled'] = false; // 売上確定
        options['livemode'] = (params.mode === 'production' ? true : false); // 売上確定

        options['customer'] = Common.sortObjectByKey({
            email: params.email || '',
            customer_name: params.customer_name ? params.customer_name : '',
            phone_number: params.tel_num || '',
            address: params.address || '',
            zip_code: params.zipcode || ''
        });
        var dest_customers = [];
        var dest_customer = Common.sortObjectByKey({
            dest_customer_name: params.dest_name ? params.dest_name : '',
            dest_zip_code: params.dest_zipcode || '',
            dest_address: params.dest_address || '',
            dest_tel: params.dest_tel_num || '',
        });
        dest_customers.push(dest_customer);
        ...
        console.log("execTranAftee");
        console.log(price);
        var checksum = Common.generateChecksumAftee(options, params.aftee_secret_key);
        if (checksum !== void 0 && checksum) {
            ...
            if (price['transaction_options'] !== void 0 && price['transaction_options'].value) {
                ...
                console.log(parseInt(price['transaction_options'].value));
            ...
            result['aftee_pub_key'] = params.aftee_pub_key;
            result['success'] = true;
        }
