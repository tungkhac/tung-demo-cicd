...
router.post('/freelance_levtech', function(req, res, next) {
    ...
    if(typeof arr_error !== 'undefined' && arr_error.length > 0) {
        ...
        var upload1 = body.upload1;
        var upload2 = body.upload2;
...
function levtechHandle(data, res) {
    ...
    (async () => {
        ...
        try {
            ...
            //upload file
            if(typeof data.upload1 != "undefined" && data.upload1.trim() != '') {
                var dirname_1 = splitImageUrl(data.upload1);
                const file_path_1 = AZURE_STORAGE_UPLOAD_URL + data.upload1;
                ...
                console.log('data.upload1=====', data.upload1);
            ...
            if(typeof data.upload2 != "undefined" && data.upload2.trim() != '') {
                var dirname_2 = splitImageUrl(data.upload2);
                const file_path_2 = AZURE_STORAGE_UPLOAD_URL + data.upload2;
                ...
                console.log('data.upload2=====', data.upload2);
                console.log('file_path_2=', file_path_2);
    ...
    (async () => {
        ...
        try {
            ...
            if(new_url == career_regist_url) {
                console.log('page redirect');
                // await page.screenshot({path: 'pictures/_levtech_333_' + Date.now() + '.png', fullPage: true});
                if(await page.$('body > div > ul') !== null){
